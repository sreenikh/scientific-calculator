...
function buildTable(fExpr, gExpr, start, end, step) {
  const rows = []
  const fn = compileFn(fExpr)
  const gn = gExpr.trim() ? compileFn(gExpr) : null
...
export default function TablePanel({ onClose }) {
  ...
  function generate() {
    ...
    try {
      const data = buildTable(fExpr.trim(), gExpr.trim(), s, e, st)
      setRows(data)
    } catch {
